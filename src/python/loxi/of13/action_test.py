...
def test_set_field(testcase, table_id, ports, property_type):
    ...
    oxm_packets = [
        (ofp.oxm.ipv4_src(value=parse_ip(ipv4)), simple_tcp_packet(ip_src=ip)),
        (ofp.oxm.ipv4_dst(value=parse_ip(ipv4)), simple_tcp_packet(ip_dst=ip)),
        (ofp.oxm.eth_src(value=parse_ip(mac)), simple_tcp_packet(eth_src=mac)),
        (ofp.oxm.eth_dst(value=parse_ip(mac)), simple_tcp_packet(eth_src=mac))
        ]
